...
class MCPManager {
  ...
  async connectServer(config: { name: string; command: string; args: string[] }): Promise<void> {
...
export function ToolResultCard({ toolName, result, error, duration }: ToolResultCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <pre className="whitespace-pre-wrap">{error || displayResult}</pre>
      </div>
...
const DEFAULT_CONFIG: MCPConfig = {
  ...
  allowedDirectories: [process.env.HOME || '', '/tmp'],
  blockedTools: [],
  servers: [
    {
      name: 'desktop-commander',
...
const testMCP = async () => {
  ...
  const listResult = await window.api.mcp.requestTool('desktop-commander', 'list_directory', {
    path: '/Users/cory.naegle/ArborChat',
    depth: 1
  })
  console.log('List result:', listResult)

  // 4. Test moderate tool (should require approval)
  const writeResult = await window.api.mcp.requestTool('desktop-commander', 'write_file', {
    path: '/tmp/test.txt',
    content: 'Hello from ArborChat!'
  })
