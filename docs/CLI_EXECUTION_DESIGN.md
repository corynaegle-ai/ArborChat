...
ipcMain.handle('cli:execute', async (event, request: {
  command: string
  workingDir?: string
...
ipcMain.handle('cli:stdin', async (_, { processId, input }: { 
  processId: string
  input: string 
}) => {
  const handle = activeProcesses.get(processId)
...
export function CommandApprovalCard({ 
  ...
  return (
    <div className={cn(
      "rounded-lg border-2 p-4 my-2",
      riskColors[command.riskLevel]
    )}>
      {/* Header */}
...
export function TerminalOutput({ processId, onCancel }: TerminalOutputProps) {
  ...
  return (
    <div className="rounded-lg border border-secondary bg-tertiary overflow-hidden my-2">
      ...
      <div 
        ref={scrollRef}
        className="p-3 font-mono text-xs max-h-64 overflow-y-auto"
      >
        {output.map((line, i) => (
          <pre 
            key={i}
