...
export function useAgent(): UseAgentResult {
  ...
  const createAgent = useCallback(
    (options: CreateAgentOptions): Agent => {
      const now = Date.now()
...
You have access to MCP tools for file operations and command execution.`
      ...
      const fullSystemPrompt =
        connected && mcpSystemPrompt ? `${basePrompt}\n\n${mcpSystemPrompt}` : basePrompt
  ...
  const addStep = useCallback(
    (type: AgentStep['type'], content: string, toolCall?: AgentStep['toolCall']): AgentStep => {
          ...
          try {
            ...
            if (alwaysApproveTools.includes(firstCall.tool)) {
              ...
              const result = await requestTool(
                firstCall.tool,
                firstCall.args,
                firstCall.explanation
              )
              const toolResultContext = formatToolResult(
                firstCall.tool,
                result.result,
                result.error
              )
  ...
  const startAgent = useCallback(
    (apiKey: string, model: string) => {
      ...
      const context = [systemMessage, { role: 'user', content: userContent }]
  ...
  const approveToolCall = useCallback(
    async (id: string, modifiedArgs?: Record<string, unknown>, apiKey?: string, model?: string) => {
